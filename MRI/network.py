...
def _match_spatial_size(source, target):
...
class Seg_Decoder_1(nn.Module):
    ...
    def forward(self, input):
        ...
        up_6 = self.up6(c5)
        up_6 = _match_spatial_size(up_6, c4)
        # print("c4 shape:", c4.shape)
        # print("up_6 shape:", up_6.shape)
        merge6 = torch.cat([up_6, c4], dim=1)
        c6=self.conv6(merge6)
        up_7=self.up7(c6)
        up_7 = _match_spatial_size(up_7, c3)
        merge7 = torch.cat([up_7, c3], dim=1)
        c7=self.conv7(merge7)
        up_8=self.up8(c7)
        up_8 = _match_spatial_size(up_8, c2)
        merge8 = torch.cat([up_8, c2], dim=1)
        c8=self.conv8(merge8)
        up_9=self.up9(c8)
        up_9 = _match_spatial_size(up_9, c1)
