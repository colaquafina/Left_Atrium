...
log_path = "/work/users/g/s/gsonw/BIOS740/final_project/AJSQ_2/logs/Gson_AJSQ_63165790.out"
...
val_pattern = re.compile(
    rf"Validation epoch\s+(\d+),\s+loss:\s+({number}),\s+"
    ...
    for line in f:
        ...
        if line.startswith("Train epoch"):
            ...
            if m:
                ...
                val_la_dice.append(float(m.group(3)))
                val_scar_dice.append(float(m.group(4)))
            else:
